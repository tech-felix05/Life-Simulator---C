...
void eat(struct Giocatore *c)
{
    ...
    printf(GREEN BOLD "1.Ristorante (in famiglia) | 2.Fast food (amici) [mangiarne troppo spesso fa male!] | 3.Mangia a casa\nScelta: " RESET);
    scanf("%d", &eatMenu);
...
void controlYearProximative(struct Giocatore *control, int *patLicense, int *brkGirlProb, int *flong, int *lvc, int *mtb, int *wrk, int *pris, int *al)
{
    ...
    if (control->age > 12 && control->age < 30)
    {
        control->fertility += 5;
    }
...
void healthAndAddiction(struct Giocatore *addictAndHealth, int *add)
{
    ...
    if (*add == 1)
    {
        addictAndHealth->healht -= 7;
        addictAndHealth->iQ -= 5;
        addictAndHealth->fertility -= 4;
        addictAndHealth->mentalHealth -= 5;
    }
    else
    {
        if (addictAndHealth->age < 25)
        {
            addictAndHealth->iQ += 2;
            addictAndHealth->fertility += 4;
            addictAndHealth->healht += 5;
        }
        if (addictAndHealth->age >= 35)
        {
            addictAndHealth->fertility -= 1;
        }
        if (addictAndHealth->age >= 70)
        {
            addictAndHealth->iQ -= 3;
            addictAndHealth->fertility -= 4;
            addictAndHealth->healht -= 3;
        }
    }
    if (addictAndHealth->iQ < 0)
    {
        addictAndHealth->iQ = 0;
    }
    if (addictAndHealth->healht > 100)
    {
        addictAndHealth->healht = 100;
    }
    if (addictAndHealth->mentalHealth > 100)
    {
        addictAndHealth->mentalHealth = 100;
    }
    else if (addictAndHealth->mentalHealth < 0)
    {
        addictAndHealth->mentalHealth = 0;
    }
}
...
void endo(struct Giocatore *endoc, int *viv, int *sem, int *countSon, int *amorCount)
{
    ...
    case 1:
      ...
      if (endoc->fertility <= 0)
      {
        ...
        if (endoc->age >= 18)
        {
            ...
            printf("\n\n[CONTROLLO DEL TUO SEME]....\n\n");

            for (int i = 0; i <= total; i++)
            {
                float gotoHundred = (float)i / total;
                float riemp = gotoHundred * bar;

                printf("\n\n[CONTROLLO DEL TUO SEME] { %d }....\n\n", i);

                printf(GREEN BOLD "\rProgresso: [" RESET);
                for (int j = 0; j < bar; j++)
                {
                    if (j < riemp)
                        printf("---<@ ");
                ...
                printf("] %d%%", (int)(gotoHundred * 100));

                fflush(stdout);
                usleep(30000);
            }

            if (endoc->fertility >= 93)
            {
                printf(GREEN "\n\n[SEME IDONEO!]\n\n" RESET);
        ...
        {
            printf("\n\n[SEI MINORENNE O HAI POCHI SOLDI]\n\n");
        }
        break;
    ...
    case 3:
        ...
        if (endoc->age >= 18 && endoc->currentPortfolio >= 1500)
        {

            if (endoc->fertility >= 55 && endoc->age < 50)
            {
